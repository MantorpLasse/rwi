"""Add the airport_aliases table (docs/architecture, "RWI - Governed
Canonical Airport Aliases - Cross-Script Identity Design" mission - see
that document for the locked design contract this migration creates
schema for; app.models.airport_alias for the exact row shape).

Additive only: creates exactly one new table (and its indexes/CHECK/FK
constraints) - `airport_aliases` - reused directly from the ORM model
(app.models.airport_alias.AirportAlias) via SQLAlchemy's own
CreateTable/CreateIndex compilation, the same technique
scripts/migrate_manual_identity_evidence.py (this repository's own most
...
schema is completely untouched (no alias storage is ever added to the
airports table itself - see that model's own docstring for why).
...
normal upgrade - admitting an airport alias remains the job of
app.services.airport_alias, never this script.

PARTIAL-SCHEMA SAFETY / CONSERVATIVE DOWNGRADE / BACKUP DISCIPLINE:
identical in shape to the manual-identity-evidence migration - `upgrade()`
...
_EXPECTED_NAMED_CONSTRAINTS = (
    "ck_airport_aliases_evidence_class",
    "ck_airport_aliases_status",
)


class IncompatibleExistingSchemaError(RuntimeError):
    """Raised by upgrade() when a table named `airport_aliases` already
    exists but its persisted shape does not match
    app.models.airport_alias's own current model - this migration never
...
def _backup_name() -> str:
    ...
    return f"runway_safe-pre-airport-alias-{timestamp}.db"
...
def downgrade(database: Path) -> None:
    ...
    table contains any row - persisted airport alias evidence is
    ...
    try:
        ...
        if count > 0:
            raise RuntimeError(
                f"downgrade() refused: {TABLE_NAME!r} contains {count} recorded alias row(s) and would "
                "be destroyed. No table was dropped."
            )
        if _table_exists(connection, TABLE_NAME):
