"""Add the source_assertion_cross_source_alias_attestations table
(docs/architecture, "RWI - Cross-Source Governed Airport Identity Binding -
Architecture Recon" mission's own Option C - see that document for the
locked design contract this migration creates schema for;
app.models.source_assertion_cross_source_alias_attestation for the exact
row shape).

Additive only: creates exactly one new table (and its indexes/FK
constraints) - `source_assertion_cross_source_alias_attestations` - reused
directly from the ORM model via SQLAlchemy's own CreateTable/CreateIndex
compilation, the same technique
scripts/migrate_airport_alias.py/scripts/migrate_airport_identifier.py
already established, reused verbatim here rather than reinvented. No
existing table, column, or row is read, changed, merged, or deleted
anywhere in this script - no ALTER TABLE appears.
...
normal upgrade - recording an attestation remains the job of
app.services.cross_source_alias_attestation, never this script.

PARTIAL-SCHEMA SAFETY / CONSERVATIVE DOWNGRADE / BACKUP DISCIPLINE:
identical in shape to migrate_airport_alias.py - `upgrade()` verifies an
already-existing table against the current ORM model rather than assuming
it is correct; `downgrade()` refuses if the table contains any row; a
timestamped backup is required before any real write.

THIS MISSION'S OWN EXPLICIT BOUNDARY: this migration is NEVER applied to
the real business database (data/runway_safe.db) within the "RWI -
Cross-Source Alias Attestation - Implementation Slice" mission - only to
temporary/fixture databases, for upgrade/downgrade/upgrade testing. See
that mission's own Phase 15/25 for the exact zero-write proof this implies.
"""
...
def _backup_name() -> str:
    ...
    return f"runway_safe-pre-cross-source-alias-attestation-{timestamp}.db"
...
def _expected_columns() -> dict:
    ...
    from the live ORM model - never hardcoded, so it can never drift."""
...
def _schema_mismatch_reasons(connection: sqlite3.Connection) -> "list[str]":
    ...
    genuinely match the current ORM model" - both `upgrade()` and
    `inspect()` call this one function, so they can never disagree."""
    reasons: "list[str]" = []
...
def inspect(database: Path) -> dict:
    ...
    never the caller's ORM engine. Never mutates anything."""
    database = database.resolve()
...
def downgrade(database: Path) -> None:
    """Refuses outright (raises, no partial change committed) if the
    table contains any row - persisted attestation evidence is permanent
    audit evidence, never discardable by a schema downgrade. Only when
    confirmed empty does this drop it."""
    connection = sqlite3.connect(database.resolve())
    try:
        connection.execute("PRAGMA foreign_keys=ON")
        connection.execute("BEGIN IMMEDIATE")
        ...
        if count > 0:
            raise RuntimeError(
                f"downgrade() refused: {TABLE_NAME!r} contains {count} recorded attestation row(s) and "
