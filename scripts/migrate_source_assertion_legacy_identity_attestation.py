"""Add the source_assertion_legacy_identity_attestations table
(docs/architecture/rwi-legacy-attached-sourceassertion-identity-governance-
design.md - see that document for the locked design contract this migration
creates schema for).

Additive only: creates exactly one new table (and its indexes/CHECK/FK
constraints) - `source_assertion_legacy_identity_attestations` - reused
directly from the ORM model
(app.models.source_assertion_legacy_identity_attestation.
SourceAssertionLegacyIdentityAttestation) via SQLAlchemy's own
CreateTable/CreateIndex compilation, the same technique
scripts/migrate_source_assertion_identity_resolution_kar1.py (this
...
upgrade - recording a human legacy identity attestation remains the job of
app.services.source_assertion_legacy_identity_attestation, never this
script.

PARTIAL-SCHEMA SAFETY / CONSERVATIVE DOWNGRADE / BACKUP DISCIPLINE:
identical in shape to the KAR1 migration - `upgrade()` verifies an
...
row; a timestamped backup is required before any real write.
"""
...
_EXPECTED_NAMED_CONSTRAINTS = (
    "ck_source_assertion_legacy_identity_attestations_action",
    "ck_source_assertion_legacy_identity_attestations_confirm_target_required",
    "ck_source_assertion_legacy_identity_attestations_target_only_for_confirm",
)


class IncompatibleExistingSchemaError(RuntimeError):
    """Raised by upgrade() when a table named
    `source_assertion_legacy_identity_attestations` already exists but its
    persisted shape does not match
    app.models.source_assertion_legacy_identity_attestation's own current
...
def _backup_name() -> str:
    ...
    return f"runway_safe-pre-legacy-identity-attestation-{timestamp}.db"
...
def downgrade(database: Path) -> None:
    ...
    contains any row - persisted human legacy-identity-attestation history
    is permanent audit evidence, never discardable by a schema downgrade.
