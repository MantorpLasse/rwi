"""Add the airport_identifiers table (docs/architecture, "RWI - Governed
Canonical Airport Identifiers - Architecture Design" mission - see that
document for the locked design contract this migration creates schema
for; app.models.airport_identifier for the exact row shape).

Additive only: creates exactly one new table (and its indexes/CHECK/FK
constraints) - `airport_identifiers` - reused directly from the ORM model
via SQLAlchemy's own CreateTable/CreateIndex compilation, the same
technique scripts/migrate_airport_alias.py (this repository's own most
recent precedent for an additive single-table migration) already
...
schema (including iata_code/icao_code/faa_code) is completely untouched
by this migration itself - those columns are only ever written later, at
runtime, by app.services.airport_identifier's own strictly-gated write
...
code columns. Admitting a governed identifier remains the job of
app.services.airport_identifier, never this script.

PARTIAL-SCHEMA SAFETY / CONSERVATIVE DOWNGRADE / BACKUP DISCIPLINE:
identical in shape to the airport-alias migration - `upgrade()` verifies
an already-existing table against the current ORM model rather than
assuming it is correct; `downgrade()` refuses if the table contains any
...
_EXPECTED_NAMED_CONSTRAINTS = (
    "ck_airport_identifiers_identifier_type",
    "ck_airport_identifiers_evidence_class",
    "ck_airport_identifiers_status",
)


class IncompatibleExistingSchemaError(RuntimeError):
    """Raised by upgrade() when a table named `airport_identifiers`
    already exists but its persisted shape does not match
    app.models.airport_identifier's own current model - this migration
...
def _backup_name() -> str:
    ...
    return f"runway_safe-pre-airport-identifier-{timestamp}.db"
...
def downgrade(database: Path) -> None:
    ...
    identifier governance evidence is permanent audit evidence, never
    discardable by a schema downgrade. Only when confirmed empty does
    this drop it. Never touches Airport.iata_code/icao_code/faa_code."""
    connection = sqlite3.connect(database.resolve())
    try:
        connection.execute("PRAGMA foreign_keys=ON")
        connection.execute("BEGIN IMMEDIATE")
        ...
        if count > 0:
            raise RuntimeError(
                f"downgrade() refused: {TABLE_NAME!r} contains {count} recorded identifier row(s) and "
