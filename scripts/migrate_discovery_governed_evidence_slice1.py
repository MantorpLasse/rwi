"""Add source_assertions.identity_guard_decision and .identity_guard_reason.

See docs/architecture/ai-discovery-governed-evidence-persistence-report.md
and docs/architecture/ai-discovery-evidence-attachment-guard.md S11/S22.
...
Both columns are added NULL on every existing row and stay NULL unless a
future app/services/discovery_evidence_persistence.py write sets them -
no existing ingestion path (NASR, USAspending, FAA acquisition) is
touched by this migration or reads/writes either column. A timestamped
backup is required before this script writes to the development
database.

No DB-level CHECK constraint bounds identity_guard_decision to the five
AttachmentOutcome values in this slice - SQLite's ADD COLUMN cannot add a
CHECK constraint as safely/simply as a plain nullable column, and doing
so would require the same full table-rebuild procedure downgrade()
already needs for DROP COLUMN, which is more than "smallest additive
migration" calls for here. The persistence service is the only writer of
this column and only ever writes a real AttachmentOutcome.value, enforced
in Python.
"""
...
NEW_COLUMNS = ("identity_guard_decision", "identity_guard_reason")


def _backup_name() -> str:
    timestamp = datetime.now(UTC).strftime("%Y%m%d-%H%M%S")
    return f"runway_safe-pre-discovery-governed-evidence-slice1-{timestamp}.db"
...
def inspect(database: Path) -> dict:
    ...
    try:
        ...
        return {
            ...
            "identity_guard_decision_column_exists": "identity_guard_decision" in columns,
            "identity_guard_reason_column_exists": "identity_guard_reason" in columns,
            "source_assertions_count": connection.execute(f"SELECT count(*) FROM {ALTERED_TABLE}").fetchone()[0],
            "source_assertions_with_identity_guard_decision": (
                connection.execute(
                    f"SELECT count(*) FROM {ALTERED_TABLE} WHERE identity_guard_decision IS NOT NULL"
                ).fetchone()[0]
                if "identity_guard_decision" in columns
                else None
            ),
...
def upgrade(database: Path) -> None:
    ...
    try:
        ...
        columns = {row[1] for row in connection.execute(f"PRAGMA table_info({ALTERED_TABLE})")}
        if "identity_guard_decision" not in columns:
            connection.execute(f"ALTER TABLE {ALTERED_TABLE} ADD COLUMN identity_guard_decision VARCHAR(30)")
        if "identity_guard_reason" not in columns:
            connection.execute(f"ALTER TABLE {ALTERED_TABLE} ADD COLUMN identity_guard_reason TEXT")
        connection.commit()
    except Exception:
        connection.rollback()
        raise
...
def _drop_column_via_rebuild(connection: sqlite3.Connection, table: str, column: str) -> None:
    ...
    Base.metadata.create_all() produces - i.e. every test fixture in this
    repository and any future clean deployment). Neither new column here
    is itself part of any foreign key, but source_assertions IS the
    target of an incoming foreign key (installation_assertion_links.assertion_id)
    - this rebuild preserves that and every other surviving constraint/
    index exactly, regardless of which column is being dropped. The
    caller must already have `PRAGMA foreign_keys=OFF` in effect (set
    outside the current transaction - SQLite ignores changes to this
    pragma made inside one)."""
