"""Add the KAR1 known-airport identity resolution table (docs/architecture/
rwi-known-airport-ambiguity-resolution-design.md - see that document for the
locked design contract, and rwi-known-airport-ambiguity-resolution-
implementation-report.md for the reviewed, committed model this migration
creates schema for).

Additive only: creates exactly one new table (and its indexes/CHECK/FK
constraints) - `source_assertion_identity_resolutions` - reused directly
from the ORM model (app.models.source_assertion_identity_resolution.
SourceAssertionIdentityResolution) via SQLAlchemy's own CreateTable/
CreateIndex compilation, the same technique
scripts/migrate_unknown_airport_candidate_relevance_reviews_erg3.py already
established as this repository's own strongest, most recent precedent for
an additive single-table migration, reused verbatim here (structure,
...
ALTER TABLE appears, and SourceAssertion's own schema is untouched (its
`airport_id` column already exists and is already nullable/writable at the
ORM level - see the design document's own S14 "no SourceAssertion schema
change needed").
...
normal upgrade - recording a human identity resolution remains the job of
app.services.source_assertion_identity_resolution, never this script.

PARTIAL-SCHEMA SAFETY / CONSERVATIVE DOWNGRADE / BACKUP DISCIPLINE:
identical in shape to the ERG3 migration - `upgrade()` verifies an
...
is never run against the real database in this implementation task (KAR1);
every test uses an isolated temp-file SQLite database (tmp_path) - the real
migration is explicitly deferred to a separate, later, explicitly-
...
from app import models as _models  # noqa: F401 - registers all metadata, including the KAR1 table
...
_EXPECTED_NAMED_CONSTRAINTS = (
    "ck_source_assertion_identity_resolutions_action",
    "ck_source_assertion_identity_resolutions_attach_target_required",
    "ck_source_assertion_identity_resolutions_target_only_for_attach",
    "fk_source_assertion_identity_resolutions_snapshot_matches_assertion",
)


class IncompatibleExistingSchemaError(RuntimeError):
    """Raised by upgrade() when a table named `source_assertion_identity_resolutions`
    already exists but its persisted shape does not match
    app.models.source_assertion_identity_resolution's own current model -
    this migration never drops, rebuilds, or otherwise silently reconciles
    an unexpected existing table; a human must resolve the collision by
    hand."""


def _backup_name() -> str:
    timestamp = datetime.now(UTC).strftime("%Y%m%d-%H%M%S")
    return f"runway_safe-pre-identity-resolution-kar1-{timestamp}.db"
...
def _expected_columns() -> dict:
    ...
    app.models.source_assertion_identity_resolution."""
...
def _schema_mismatch_reasons(connection: sqlite3.Connection) -> "list[str]":
    ...
    if actual_columns != expected_columns:
        reasons.append(
            f"columns do not match the expected KAR1 schema - expected {expected_columns!r}, "
    ...
    if actual_fks != expected_fks:
        reasons.append(
            f"foreign keys do not match the expected KAR1 schema - expected {expected_fks!r}, "
...
def _verify_existing_table_matches_expected_schema(connection: sqlite3.Connection) -> None:
    ...
    if reasons:
        raise IncompatibleExistingSchemaError(
            f"table {TABLE_NAME!r} already exists but is incompatible with the expected KAR1 schema: "
...
def downgrade(database: Path) -> None:
    ...
    contains any row - persisted human identity-resolution history is
    ...
    try:
        ...
        if count > 0:
            raise RuntimeError(
                f"downgrade() refused: {TABLE_NAME!r} contains {count} recorded identity-resolution "
