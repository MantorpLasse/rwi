"""Add the signal_publication_actions table ("RWI - Signal Publication
Governance - Design + Implementation" mission - see
app.models.signal_publication_action for the exact row shape this migration
creates schema for).

Additive only: creates exactly one new table (and its indexes/CHECK/FK
constraints) - `signal_publication_actions` - reused directly from the ORM
model via SQLAlchemy's own CreateTable/CreateIndex compilation, the same
technique scripts/migrate_manual_claim_evidence.py/
...
row is read, changed, merged, or deleted anywhere in this script - in
particular, `signals.published` (added by
scripts/migrate_signal_publication_slice9a.py) is never touched.
...
upgrade - recording a publish/unpublish action remains the job of
app.services.signal_publication, never this script.

PARTIAL-SCHEMA SAFETY / CONSERVATIVE DOWNGRADE / BACKUP DISCIPLINE: identical
in shape to migrate_manual_claim_evidence.py - `upgrade()` verifies an
already-existing table against the current ORM model rather than assuming it
is correct; `downgrade()` refuses if the table contains any row; a
timestamped backup is required before any real write.

THIS MISSION'S OWN EXPLICIT BOUNDARY: this migration is NEVER applied to the
real business database (data/runway_safe.db) within this mission - only to
temporary/fixture databases, for upgrade/downgrade/upgrade testing.
"""
...
_EXPECTED_NAMED_CONSTRAINTS = (
    "ck_signal_publication_actions_action",
)
...
def _backup_name() -> str:
    ...
    return f"runway_safe-pre-signal-publication-action-{timestamp}.db"
