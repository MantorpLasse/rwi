"""Add source_assertions.promotion_policy_decision and .promotion_policy_reason.

See docs/architecture/promotion-policy-persistence-slice7-report.md and
docs/architecture/evidence-to-signal-semantics-design.md S17/S18 (extended by
docs/architecture/signal-promotion-policy-slice5-design.md S25's own
recommended slice 7). Additive only: two new nullable columns on the
existing source_assertions table (the same table
scripts/migrate_discovery_governed_evidence_slice1.py added
identity_guard_decision/identity_guard_reason to, and
scripts/migrate_intelligence_review_persistence_slice4.py added
intelligence_review_decision/intelligence_review_reason to). No existing row
in any table is read, changed, merged, or deleted. Both columns are added
NULL on every existing row and stay NULL unless a future
app/services/promotion_policy_persistence.py write sets them - no existing
ingestion path (NASR, USAspending, FAA acquisition) and no identity-guard or
intelligence-review write path is touched by this migration or reads/writes
either column. A timestamped backup is required before this script writes to
the development database.

No DB-level CHECK constraint bounds promotion_policy_decision to the three
PromotionPolicyOutcome values in this slice - SQLite's ADD COLUMN cannot add
a CHECK constraint as safely/simply as a plain nullable column, and doing so
would require the same full table-rebuild procedure downgrade() already
needs for DROP COLUMN, which is more than "smallest additive migration"
calls for here (identical reasoning to the two prior additive column pairs
on this same table). The persistence service is the only writer of this
column and only ever writes a real PromotionPolicyOutcome.value, enforced in
Python.
"""
...
NEW_COLUMNS = ("promotion_policy_decision", "promotion_policy_reason")


def _backup_name() -> str:
    timestamp = datetime.now(UTC).strftime("%Y%m%d-%H%M%S")
    return f"runway_safe-pre-promotion-policy-persistence-slice7-{timestamp}.db"
...
def inspect(database: Path) -> dict:
    ...
    try:
        ...
        return {
            ...
            "promotion_policy_decision_column_exists": "promotion_policy_decision" in columns,
            "promotion_policy_reason_column_exists": "promotion_policy_reason" in columns,
            "source_assertions_count": connection.execute(f"SELECT count(*) FROM {ALTERED_TABLE}").fetchone()[0],
            "source_assertions_with_promotion_policy_decision": (
                connection.execute(
                    f"SELECT count(*) FROM {ALTERED_TABLE} WHERE promotion_policy_decision IS NOT NULL"
                ).fetchone()[0]
                if "promotion_policy_decision" in columns
                else None
            ),
...
def upgrade(database: Path) -> None:
    ...
    try:
        ...
        columns = {row[1] for row in connection.execute(f"PRAGMA table_info({ALTERED_TABLE})")}
        if "promotion_policy_decision" not in columns:
            connection.execute(f"ALTER TABLE {ALTERED_TABLE} ADD COLUMN promotion_policy_decision VARCHAR(30)")
        if "promotion_policy_reason" not in columns:
            connection.execute(f"ALTER TABLE {ALTERED_TABLE} ADD COLUMN promotion_policy_reason TEXT")
        connection.commit()
    except Exception:
        connection.rollback()
        raise
...
def _drop_column_via_rebuild(connection: sqlite3.Connection, table: str, column: str) -> None:
    ...
    scripts/migrate_intelligence_review_persistence_slice4.py::_drop_column_via_rebuild()
    (itself reused from scripts/migrate_discovery_governed_evidence_slice1.py,
    ultimately from scripts/migrate_canonical_runway_runway_end_slice1.py,
