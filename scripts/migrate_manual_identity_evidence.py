"""Add the manual_identity_evidences table (docs/architecture, "RWI - New
Source Family Manual Identity Evidence - Architecture Design" mission - see
that document for the locked design contract this migration creates schema
for; app.models.manual_identity_evidence for the exact row shape).

Additive only: creates exactly one new table (and its indexes/CHECK/FK
constraints) - `manual_identity_evidences` - reused directly from the ORM
model (app.models.manual_identity_evidence.ManualIdentityEvidence) via
SQLAlchemy's own CreateTable/CreateIndex compilation, the same technique
scripts/migrate_source_assertion_legacy_identity_attestation.py (this
repository's own most recent precedent for an additive single-table
migration) already established, reused verbatim here rather than
reinvented. No existing table, column, or row is read, changed, merged, or
deleted anywhere in this script - no ALTER TABLE appears, and
SourceAssertion's own schema is completely untouched.

NO BACKFILL, EVER: `upgrade()` never inserts a row into the new table under
any circumstance. The table is always empty immediately after a normal
upgrade - recording a human's manual identity evidence remains the job of
app.services.manual_identity_evidence, never this script.

PARTIAL-SCHEMA SAFETY / CONSERVATIVE DOWNGRADE / BACKUP DISCIPLINE:
identical in shape to the legacy-identity-attestation migration -
`upgrade()` verifies an already-existing table against the current ORM
model rather than assuming it is correct; `downgrade()` refuses outright if
the table contains any row; a timestamped backup is required before any
real write.
"""
...
TABLE_NAME = "manual_identity_evidences"

_EXPECTED_NAMED_CONSTRAINTS = (
    "ck_manual_identity_evidences_extraction_mode",
)


class IncompatibleExistingSchemaError(RuntimeError):
    """Raised by upgrade() when a table named `manual_identity_evidences`
    already exists but its persisted shape does not match
    app.models.manual_identity_evidence's own current model - this
...
def _backup_name() -> str:
    ...
    return f"runway_safe-pre-manual-identity-evidence-{timestamp}.db"
...
def _expected_columns() -> dict:
    ...
    app.models.manual_identity_evidence."""
...
def downgrade(database: Path) -> None:
    ...
    contains any row - persisted manual identity evidence is permanent
    ...
    try:
        ...
        if count > 0:
            raise RuntimeError(
                f"downgrade() refused: {TABLE_NAME!r} contains {count} recorded evidence row(s) and "
