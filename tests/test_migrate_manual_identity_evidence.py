"""Tests for scripts/migrate_manual_identity_evidence.py (docs/architecture,
...
tests/test_source_assertion_legacy_identity_attestation_migration.py, this
...
import scripts.migrate_manual_identity_evidence as migration
...
def _seed_ungoverned_assertion(db_path):
    """Seeds one Airport, one Source, and one SourceAssertion shaped
    exactly like the eligible class for this mechanism (airport_id set,
    identity_guard_decision NULL, no EvidenceBag) - the minimum realistic
    starting state."""
    ...
    with Session(engine) as s:
        ...
        assertion = SourceAssertion(
            ...
            raw_relevant_text="Test Airport in Test City is getting a new EMAS installation.",
            source_record_identifier="rec-1", evidence_quality="direct_strong", review_state="unreviewed",
        )
        s.add(assertion)
        s.commit()
        return source.id, airport.id, assertion.id
...
def test_upgrade_preserves_pre_existing_data(tmp_path):
    db_path = tmp_path / "test.db"
    _pre_migration_db(db_path)
    _source_id, airport_id, assertion_id = _seed_ungoverned_assertion(db_path)
...
def test_downgrade_refuses_when_rows_exist(tmp_path):
    db_path = tmp_path / "test.db"
    _pre_migration_db(db_path)
    source_id, _airport_id, assertion_id = _seed_ungoverned_assertion(db_path)
    migration.upgrade(db_path)

    engine = create_engine(f"sqlite:///{db_path}")
    with Session(engine) as s:
        record_manual_identity_evidence(
            s, source_assertion_id=assertion_id, source_id=source_id,
            evidence_excerpt="Test Airport in Test City is getting a new EMAS installation.",
...
def test_upgrade_downgrade_upgrade_cycle(tmp_path):
    """Explicit Phase 17 requirement: upgrade -> downgrade -> upgrade,
    with foreign_key_check/integrity_check clean at each stage, on an
    isolated temp database only."""
    db_path = tmp_path / "test.db"
    _pre_migration_db(db_path)
