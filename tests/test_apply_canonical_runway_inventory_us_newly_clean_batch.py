"""Isolated tests for scripts/apply_canonical_runway_inventory_us_newly_clean_batch.py
(docs/domain/canonical-runway-us-newly-clean-12-batch-report.md).

Never touches the real development database - builds a synthetic NASR zip
covering five airports: MDW/CGF already fully populated by a prior batch
(must stay untouched/excluded), TST a newly-clean airport whose NASR data
includes a helipad row mixed in with its real runway (proves the
special-record classification rule is what makes it newly-clean, not
ALREADY_COMPLETE), BAD an airport with only a helipad row (must classify
UNRESOLVED, never touched), and NOID an airport with no FAA/IATA/ICAO
identifier (must be excluded, never touched)."""
import hashlib
import json
import zipfile
...
from scripts.apply_canonical_runway_inventory_us_newly_clean_batch import backup_database, dry_run, run
...
RWY_ROWS = [
    ...
    "2026-08-06,09999.*A,APT,XX,TST,TESTVILLE,US,H1,50,50,CONC,,NONE,,,,,,,,,,,",
    "2026-08-06,08888.*A,APT,XX,BAD,BADTOWN,US,H1,50,50,CONC,,NONE,,,,,,,,,,,",
]
RWY_END_ROWS = [
    ...
    "2026-08-06,09999.*A,APT,XX,TST,TESTVILLE,US,H1,H1,0",
    "2026-08-06,08888.*A,APT,XX,BAD,BADTOWN,US,H1,H1,0",
]

# The approved snapshot for this fixture: MDW/CGF already fully populated
# -> ALREADY_COMPLETE, excluded. TST is newly-clean once its H1 row is
# filtered out -> +1 runway/+2 ends. BAD has only a helipad row -> UNRESOLVED,
# excluded. NOID has no identifier -> excluded.
EXPECTED_AIRPORT_COUNT = 1
EXPECTED_CREATES = 1
EXPECTED_ENRICH = 0
...
def _seed_pre_apply_state(session: Session) -> dict[str, Airport]:
    ...
    noid = Airport(name="No Identifier Field", country="USA")  # faa/iata/icao all None
    session.add_all([mdw, cgf, tst, bad, noid])
    session.flush()
    ...
    bad_legacy_runway = Runway(airport=bad, designation="OLD-LEGACY")
    ...
    session.commit()
    return {"mdw": mdw, "cgf": cgf, "tst": tst, "bad": bad, "noid": noid}
...
def test_dry_run_matches_the_expected_newly_clean_shape(tmp_path):
    zip_path = _synthetic_zip(tmp_path)
    with Session(_engine()) as session:
        _seed_pre_apply_state(session)
        report = dry_run(session, zip_path=zip_path)

    assert report["aggregate"]["airport_count"] == EXPECTED_AIRPORT_COUNT
    ...
    # MDW/CGF are ALREADY_COMPLETE - clean overall, but not "newly clean"
    # (not in airport_ids), and not "excluded" either (that label is only
    # for genuinely non-clean classifications).
    only_tst = report["airport_ids"]
    assert len(only_tst) == 1  # only TST has pending work
    # BAD (helipad-only) and NOID (no identifier) are genuinely excluded, both UNRESOLVED.
    excluded_classifications = {e["classification"] for e in report["excluded"]}
    assert excluded_classifications == {"UNRESOLVED"}
...
def test_first_apply_touches_only_the_newly_clean_airport_and_protects_everything_else(tmp_path):
    zip_path = _synthetic_zip(tmp_path)
    engine = _engine()
    with Session(engine) as session:
        ...
        result = run(
            ...
            apply=True,
            zip_path=zip_path,
            expected_airport_count=EXPECTED_AIRPORT_COUNT,
    ...
    with Session(engine) as session:
        ...
        assert tst_runways[0].designation == "9/27"  # the H1 row never became a Runway
        assert len(session.scalars(select(RunwayEnd).where(RunwayEnd.runway_id == tst_runways[0].id)).all()) == 2

        # BAD (UNRESOLVED - only a helipad row) must be completely untouched.
        ...
        # MDW/CGF (the already-applied 63-airport batch's shape) untouched.
        ...
        assert len(session.scalars(select(Airport)).all()) == 5
...
def test_apply_aborts_when_membership_changes_before_write(tmp_path):
    """Simulates the database/classification changing out from under the
    script between the initial resolve and the immediate pre-write
    re-check - airport 2 was newly-clean on the first pass, excluded
    (e.g. reclassified AMBIGUOUS) on the second."""
    zip_path = _synthetic_zip(tmp_path)
    engine = _engine()
    first = [
    ...
    with Session(engine) as session:
        before = _counts(session)
        with patch(
            "scripts.apply_canonical_runway_inventory_us_newly_clean_batch.resolve_us_clean_batch",
