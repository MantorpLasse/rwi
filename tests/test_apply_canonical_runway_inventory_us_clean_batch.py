...
def test_public_export_after_apply_publishes_banor_and_leaks_nothing(tmp_path):
    ...
    isolated database, the static export must publish the newly-created
    canonical Runway inventory under "Banor" (docs/product/public-canonical-
    runway-inventory-report.md) while still leaking no RunwayEnd/
    runway_end_id internals anywhere."""
    zip_path = _synthetic_zip(tmp_path)
    engine = _engine()
    with Session(engine) as session:
        _seed_pre_apply_state(session)
