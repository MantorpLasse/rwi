"""Tests for scripts/migrate_intelligence_review_persistence_slice4.py
...
tests/test_discovery_governed_evidence_migration.py, including the
...
from scripts.migrate_intelligence_review_persistence_slice4 import downgrade, inspect, upgrade


def _pre_migration_database(tmp_path: Path) -> Path:
    """Every current table, with source_assertions hand-built in its
    pre-slice4 shape - i.e. including identity_guard_decision/
    identity_guard_reason (added by Slice 1, already part of the current
    model) but NOT intelligence_review_decision/intelligence_review_reason
    (this slice's own addition)."""
    database = tmp_path / "pre_slice4.db"
    ...
    # test_discovery_governed_evidence_migration.py's own technique, so the
    # byte-for-byte schema-text comparison in test_downgrade_is_exactly_reversible
    # is meaningful rather than an artifact of two different construction paths.
...
def _seed_pre_migration_assertion(database: Path) -> None:
    ...
    connection.execute(
        ...
        "parser_identifier, evidence_quality, review_state, identity_guard_decision, identity_guard_reason, created_at) "
...
def test_upgrade_adds_only_the_expected_columns(tmp_path):
    ...
    assert after["intelligence_review_decision_column_exists"] is True
    assert after["intelligence_review_reason_column_exists"] is True
    assert after["source_assertions_count"] == 1
    assert after["source_assertions_with_intelligence_review_decision"] == 0
    ...
    row = connection.execute(
        "SELECT id, source_id, assertion_type, identity_guard_decision, identity_guard_reason, "
        "intelligence_review_decision, intelligence_review_reason, created_at "
        "FROM source_assertions WHERE id=1"
    ).fetchone()
    connection.close()

    assert row == (1, 1, "project_construction", "ATTACH_CONFIRMED", "test identity reason", None, None, "2026-08-19 00:00:00")
...
def test_downgrade_is_exactly_reversible(tmp_path):
    ...
    row_before = connection.execute(
        ...
        "created_at FROM source_assertions WHERE id=1"
    ).fetchone()
    connection.close()
    ...
    after = inspect(database)
    assert after["intelligence_review_decision_column_exists"] is False
    assert after["intelligence_review_reason_column_exists"] is False
    assert after["source_assertions_count"] == 1
...
# too, now that source_assertions ALSO already carries the Slice 1
# identity_guard_* columns pre-migration.
# ---------------------------------------------------------------------------
...
def test_downgrade_succeeds_against_the_full_realistic_schema_with_an_incoming_foreign_key(tmp_path):
    ...
    time against source_assertions after it already carries the Slice 1
    identity_guard_* columns."""
    database = _pre_migration_database(tmp_path)
    ...
    connection.execute("PRAGMA foreign_keys=ON")
    connection.execute(
        "UPDATE source_assertions SET intelligence_review_decision='REVIEW_REQUIRED', "
        "intelligence_review_reason='test intelligence reason text' WHERE id=1"
    )
    connection.commit()
    ...
    assert assertion_row == (
        ...
        "direct_strong", "unreviewed", "ATTACH_CONFIRMED", "test identity reason", "2026-08-19 00:00:00",
    )
    ...
    assert final["intelligence_review_decision_column_exists"] is True
    assert final["intelligence_review_reason_column_exists"] is True
